...
describe("html extension redirects (Ahrefs File 15)", () => {
  ...
  it("Worker extensionlessPPathToHtml maps indexable utilities only (Phase 2B)", () => {
  ...
  it("generateStaticSite writes _redirects covering Ahrefs File 15 shapes", () => {
    ...
    // Phase 2 legacy /p/ → canonical article.
    expect(body).toContain(
      "/p/youtube-thumbnail-url.html /2026/08/youtube-thumbnail-url.html 301",
    );
    expect(body).toContain("/p/youtube-thumbnail-extractor / 301");
    expect(body).toContain("/p/copyright.html /copyright 301");
    // Never redirect an already-.html source from the walk (would loop) — legacy .html sources are explicit.
