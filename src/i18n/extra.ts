...
const EN: Record<ExtraKey, string> = {
  ...
  channelHint: "Paste one public video URL per line. Channel-only and playlist-only links are not supported.",
  trustAbout: "About",
...
const AR: Partial<Record<ExtraKey, string>> = {
  ...
  channelHint: "الصق رابط فيديو عام واحد في كل سطر. روابط القناة وقوائم التشغيل غير مدعومة.",
  trustAbout: "من نحن",
...
const FR: Partial<Record<ExtraKey, string>> = {
  ...
  compare: "Comparer",
  channelHint: "Collez une URL vidéo publique par ligne. Les liens de chaîne ou de playlist seuls ne sont pas pris en charge.",
  trustAbout: "À propos",
...
const ES: Partial<Record<ExtraKey, string>> = {
  ...
  compare: "Comparar",
  channelHint: "Pega una URL de vídeo pública por línea. Los enlaces solo de canal o lista de reproducción no están compatibles.",
...
const DE: Partial<Record<ExtraKey, string>> = {
  ...
  compare: "Vergleichen",
  channelHint: "Fügen Sie pro Zeile eine öffentliche Video-URL ein. Reine Kanal- oder Playlist-Links werden nicht unterstützt.",
  trustAbout: "Über uns",
  trendingTags: "Trending-Tags",
...
const PT: Partial<Record<ExtraKey, string>> = {
  ...
  compare: "Comparar",
  channelHint: "Cole um URL de vídeo público por linha. Links apenas de canal ou playlist não são suportados.",
  trustAbout: "Sobre",
...
const RU: Partial<Record<ExtraKey, string>> = {
  ...
  compare: "Сравнить",
  channelHint: "Вставьте один публичный URL видео в каждой строке. Ссылки только на канал или плейлист не поддерживаются.",
